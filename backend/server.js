...
const TRUSTED_SDS_DOMAINS = [
  'cloroxpro.com', 'thecloroxcompany.com', 'diversey.com', 'ecolab.com', 'grainger.com',
  'sds.chemtel.net', 'fisher.com', 'fishersci.com', 'sigmaaldrich.com', 'uline.com',
  'img.uline.com', 'simplegreen.com', 'cdn.simplegreen.com', 'rbnainfo.com',
  'crcindustries.com', 'markem-imaje.com', 'wd40.com', 'homedepot-static.com', 'images.thdstatic.com'
];
...
async function fetchJson(url, options = {}) {
  const res = await fetch(url, {
    ...options,
    headers: { Accept: 'application/json', 'User-Agent': 'ChemicalSearchBackend/1.0', ...(options.headers || {}) }
  });
...
async function fetchTextOrPdf(url, options = {}) {
  const res = await fetch(url, {
    ...options,
    headers: { 'User-Agent': 'ChemicalSearchBackend/1.0', ...(options.headers || {}) }
  });
  if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
...
function extractSdsFields(text, url) {
  ...
  return {
    chemical_name: lineAfterLabel(lines, ['Product name', 'Product identifier', 'Product Identity', 'Material name', 'Chemical name']),
    manufacturer: lineAfterLabel(lines, ['Manufacturer', 'Supplier', 'Company', 'Distributed by', 'Responsible party']),
    product_code: lineAfterLabel(lines, ['Product code', 'Product number', 'Item number', 'Part number', 'SDS number']),
    cas_number: casMatches[0] || '',
    composition: extractComposition(raw),
    sds_url: url
  };
}
...
function buildSearchQuery(term, fields = {}) {
  return [firstUseful(fields.chemical_name, fields.product_name, term), fields.product_code, fields.manufacturer, 'SDS PDF safety data sheet'].filter(Boolean).join(' ');
}
...
async function lookupSdsSearch(term, fields = {}) {
  ...
  return {
    ...
    fields: {},
    notes: ['Product-specific SDS search requires BRAVE_SEARCH_API_KEY or GOOGLE_CSE_API_KEY + GOOGLE_CSE_CX.'],
...
async function readLocalApproved() {
  ...
  const raw = await fs.readFile(LOCAL_APPROVED_PATH, 'utf8');
  try {
...
async function commitApprovedToGithub(record) {
  ...
  const filePath = process.env.GITHUB_APPROVED_FILE || 'chemicalsearch-site/sds-approved.js';
  ...
  if (getRes.ok) {
    ...
    if (match) {
      try { records = JSON.parse(match[1]); } catch { records = []; }
    }
  }
  records.push(record);
...
app.post('/api/autofill', async (req, res) => {
  try {
    ...
    res.json({ query: term, ...result, warning: 'Autofill is a review aid only. Always verify against the current official SDS before adding or using safety data.' });
  } catch (error) {
...
app.post('/api/submit-request', async (req, res) => {
  try {
    ...
    if (!request.chemical_name && !request.sds_url) return res.status(400).json({ error: 'Chemical/product name or SDS link is required.' });
    ...
    const powerPayload = { ...request, secret: process.env.POWER_AUTOMATE_SHARED_SECRET || '' };
    const flowRes = await fetch(webhookUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(powerPayload)
    });
    if (!flowRes.ok) throw new Error(`Power Automate webhook failed: ${flowRes.status} ${await flowRes.text()}`);
    res.json({ ok: true, queued: true, request_id: request.request_id, message: 'Request sent to Teams for supervisor review.' });
  } catch (error) {
...
app.post('/api/review-callback', async (req, res) => {
  try {
    ...
    if (expectedSecret && providedSecret !== expectedSecret) return res.status(401).json({ error: 'Unauthorized review callback.' });

    const decision = clean(req.body.decision).toLowerCase();
    if (!['approved', 'approve', 'denied', 'deny', 'rejected', 'reject'].includes(decision)) return res.status(400).json({ error: 'Decision must be approved or denied.' });

    if (['denied', 'deny', 'rejected', 'reject'].includes(decision)) {
      return res.json({ ok: true, decision: 'denied', request_id: req.body.request_id, message: 'Denial received. No chemical record was added.' });
    }

    const record = normalizeApprovedChemical(req.body);
    if (!record.name || !record.sds_url) return res.status(400).json({ error: 'Approved chemical requires at least name and SDS URL.' });
    await appendLocalApproved(record);
    const github = await commitApprovedToGithub(record);
    res.json({ ok: true, decision: 'approved', record, github });
  } catch (error) {
