...
function titleFromHtml(raw = '') {
  ...
  for (const item of matches) {
    const candidate = cleanupNameCandidate(stripHtml(item || ''));
    if (candidate.length >= 3 && !/^(sds|msds|viewer|safety data sheet)$/i.test(candidate)) return candidate;
  }
  return '';
}
  ...
  try {
    ...
    for (const [key, value] of parsed.searchParams.entries()) {
      if (URL_NAME_KEYS.includes(key.toLowerCase())) {
        const candidate = cleanupNameCandidate(value);
        if (candidate.length >= 3) return candidate;
      }
    }
    for (const value of parsed.searchParams.values()) {
      const candidate = cleanupNameCandidate(value);
      if (/[a-zA-Z]{3,}/.test(candidate) && !/^\d+$/.test(candidate)) return candidate;
    }
    const last = decodeURIComponent(parsed.pathname.split('/').filter(Boolean).pop() || '');
    const candidate = cleanupNameCandidate(last);
    if (candidate.length >= 3 && !/^(sdsviewer|viewer|view|download|index|file)$/i.test(candidate)) return candidate;
    return '';
  } catch { return ''; }
}
...
function extractSdsFields(text, url) {
  ...
  const manufacturer = firstUseful(lineAfterLabel(lines, ['Manufacturer', 'Supplier', 'Company', 'Distributed by', 'Responsible party', 'Company name', 'Manufacturer/Supplier']), regexPick(raw, [/(?:Manufacturer|Supplier|Company|Distributed by|Responsible party|Company name)\s*[:#\-–—]\s*([^\n\r]{2,160})/i]));
  const productCode = firstUseful(lineAfterLabel(lines, ['Product code', 'Product number', 'Item number', 'Part number', 'SDS number', 'Material number', 'Catalog number']), regexPick(raw, [/(?:Product code|Product number|Item number|Part number|SDS number|Material number|Catalog number)\s*[:#\-–—]\s*([^\n\r]{2,80})/i]));
...
async function lookupSubmittedSds(url) {
  ...
  return { confidence: isTrustedSdsUrl(url) ? Math.min(0.97, 0.72 + extractedCount * 0.05) : Math.min(0.86, 0.55 + extractedCount * 0.06), source: 'Submitted SDS document', source_type: 'sds_document_parse', fields, notes: [`Parsed submitted SDS link: ${url}`, extractedCount ? `Extracted ${extractedCount} field(s) from SDS text/metadata.` : 'Read the SDS link, but could not extract structured fields from its text or metadata. Try adding product name/manufacturer or use manual review.', 'Verify all extracted values against the visible SDS before submitting.'], links: [{ label: 'Submitted SDS', url }] };
}

function findCas(synonyms = []) { return synonyms.map(clean).find((item) => CAS_SINGLE_PATTERN.test(item)) || ''; }
async function lookupPubChem(term) { if (/^https?:\/\//i.test(term)) return null; const encoded = encodeURIComponent(term); const cidData = await fetchJson(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/${encoded}/cids/JSON`); const cid = cidData?.IdentifierList?.CID?.[0]; if (!cid) return null; const [propertiesData, synonymsData] = await Promise.all([fetchJson(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${cid}/property/Title,MolecularFormula,MolecularWeight,IUPACName,CanonicalSMILES/JSON`), fetchJson(`https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${cid}/synonyms/JSON`).catch(() => null)]); const properties = propertiesData?.PropertyTable?.Properties?.[0] || {}; return { confidence: CAS_SINGLE_PATTERN.test(term) ? 0.82 : 0.7, source: 'PubChem', source_type: 'chemical_identity', fields: { chemical_name: firstUseful(properties.Title, term), cas_number: findCas(synonymsData?.InformationList?.Information?.[0]?.Synonym || []), composition: properties.MolecularFormula || '' }, notes: [`PubChem CID ${cid}`, `PubChem URL: https://pubchem.ncbi.nlm.nih.gov/compound/${cid}`, properties.IUPACName ? `IUPAC name: ${properties.IUPACName}` : '', properties.MolecularWeight ? `Molecular weight: ${properties.MolecularWeight}` : '', 'Chemical identity only. Product-specific SDS, supplier, and product code require official SDS verification.'].filter(Boolean), links: [{ label: 'PubChem', url: `https://pubchem.ncbi.nlm.nih.gov/compound/${cid}` }] }; }
...
function parseSearchResults(results, provider, query) { const candidates = results.map((item) => ({ title: item.title || 'SDS result', url: item.url || item.link || '', description: item.description || item.snippet || '' })).filter((item) => item.url); const best = candidates.find((item) => isTrustedSdsUrl(item.url)) || candidates.find((item) => /\.pdf($|\?)/i.test(item.url)) || candidates[0]; if (!best) return { confidence: 0.2, source: provider, source_type: 'sds_search', fields: {}, notes: [`No SDS search result found for: ${query}`], links: [] }; return { confidence: isTrustedSdsUrl(best.url) ? 0.78 : /\.pdf($|\?)/i.test(best.url) ? 0.62 : 0.48, source: provider, source_type: 'sds_search', fields: { sds_url: best.url }, notes: [`Possible SDS result from ${provider}: ${best.title}`, best.description, isTrustedSdsUrl(best.url) ? 'Domain is on the trusted SDS/source allowlist.' : 'Result requires manual verification before use.'].filter(Boolean), links: candidates.slice(0, 5).map((item) => ({ label: item.title, url: item.url })) }; }
async function lookupSdsSearch(term, fields = {}) { const query = buildSearchQuery(term, fields); if (process.env.BRAVE_SEARCH_API_KEY) return parseSearchResults((await fetchJson(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=5`, { headers: { 'X-Subscription-Token': process.env.BRAVE_SEARCH_API_KEY } }))?.web?.results || [], 'Brave Search', query); if (process.env.GOOGLE_CSE_API_KEY && process.env.GOOGLE_CSE_CX) { const data = await fetchJson(`https://www.googleapis.com/customsearch/v1?key=${encodeURIComponent(process.env.GOOGLE_CSE_API_KEY)}&cx=${encodeURIComponent(process.env.GOOGLE_CSE_CX)}&q=${encodeURIComponent(query)}&num=5`); return parseSearchResults((data?.items || []).map((item) => ({ title: item.title, url: item.link, description: item.snippet })), 'Google Custom Search', query); } return { confidence: 0.25, source: 'SDS search not configured', source_type: 'sds_search_guidance', fields: {}, notes: ['Product-specific SDS search requires BRAVE_SEARCH_API_KEY or GOOGLE_CSE_API_KEY + GOOGLE_CSE_CX.'], links: [{ label: 'Manual SDS search', url: `https://www.google.com/search?q=${encodeURIComponent(query)}` }] }; }
function mergeCandidates(candidates = []) { const merged = { confidence: 0, fields: {}, notes: [], sources: [], links: [], needs_verification: true }; for (const candidate of candidates.filter(Boolean)) { merged.confidence = Math.max(merged.confidence, candidate.confidence || 0); merged.sources.push({ source: candidate.source, source_type: candidate.source_type, confidence: candidate.confidence }); Object.entries(candidate.fields || {}).forEach(([key, value]) => { if (!clean(merged.fields[key]) && clean(value)) merged.fields[key] = value; }); merged.notes.push(...(candidate.notes || [])); merged.links.push(...(candidate.links || [])); } merged.notes = [...new Set(merged.notes.map(clean).filter(Boolean))]; const seenLinks = new Set(); merged.links = merged.links.filter((link) => { if (!link?.url || seenLinks.has(link.url)) return false; seenLinks.add(link.url); return true; }); return merged; }
...
app.post('/api/autofill', async (req, res) => { try { const fields = req.body.fields || req.body; const term = buildLookupTerm(fields); if (term.length < 2) return res.status(400).json({ error: 'Enter at least one product, CAS, supplier, composition, or SDS field.' }); const candidates = []; await Promise.allSettled([clean(fields.sds_url) ? lookupSubmittedSds(fields.sds_url).then((r) => r && candidates.push(r)).catch((e) => candidates.push({ confidence: 0.1, source: 'Submitted SDS document', source_type: 'sds_document_parse_error', fields: { sds_url: fields.sds_url }, notes: [`Could not read SDS link: ${e.message}`], links: [{ label: 'Submitted SDS', url: fields.sds_url }] })) : Promise.resolve(), lookupPubChem(term).then((r) => r && candidates.push(r)).catch(() => null), lookupSdsSearch(term, fields).then((r) => r && candidates.push(r)).catch(() => null)]); res.json({ query: term, ...mergeCandidates(candidates), warning: 'Autofill is a review aid only. Always verify against the current official SDS before adding or using safety data.' }); } catch (error) { res.status(500).json({ error: error.message || 'Autofill failed' }); } });
