...
```text
chemicalsearch-site/index.html          Static shell plus style/script loading only
chemicalsearch-site/runtime-config.js   Runtime API URL override setup from localStorage
chemicalsearch-site/app-base.css        Base app styles extracted from the old app.js injected style block
chemicalsearch-site/styles.css          Layout overrides, final fixes, print styles, and home button styling
chemicalsearch-site/sojo-theme.css      SOJO-inspired visual theme extracted from the old enhancements layer
chemicalsearch-site/app.js              Main app rendering, routing, search, detail views, fallback add/update route, and request receipt rendering
chemicalsearch-site/autofill-client.js  Production add/update request form, backend autofill calls, request submission, and local request saving
...
chemicalsearch-site/sds-approved.js     Approved records generated by the review workflow through GitHub writeback
```
...
docs/STABLE_STATE.md       Current non-secret production-stability checkpoint
```
...
5. Autofill works or fails gracefully.
6. Blank or partial request can be submitted.
7. Teams approval card appears.
8. Approved request updates GitHub.
9. Frontend redeploy shows approved chemical.
```
...
2. Add lightweight validation for frontend asset loading and backend syntax checks.
3. Clarify frontend file responsibilities before changing route/form ownership.
4. Add a minimal smoke test or health-check script.
5. Split backend/server.js into smaller route and service modules only after the approval workflow has tests or a reliable manual checkpoint.
```
